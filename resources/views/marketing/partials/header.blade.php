    ...
    <div class="hidden flex-1 justify-center lg:flex">
      <div class="flex items-center gap-x-2">
        <a href="{{ route('marketing.why.index') }}" class="group flex items-center gap-x-2 rounded-sm border border-b-3 border-transparent px-2 py-1 transition-colors duration-150 hover:border-gray-400 hover:bg-white">
          ...
          <p class="text-sm text-gray-700 group-hover:text-gray-900">Why PeopleOS</p>
        </a>
        ...
        <a href="" class="group flex items-center gap-x-2 rounded-sm border border-b-3 border-transparent px-2 py-1 transition-colors duration-150 hover:border-gray-400 hover:bg-white">
          <x-phosphor-users-three class="h-4 w-4 text-rose-600 group-hover:text-rose-700" />
      ...
      <div class="relative ms-3 flex items-center gap-x-3">
        ...
        <x-dropdown align="right" width="48">
          ...
          <x-slot name="content">
            ...
            <x-dropdown-link href="{{ route('administration.index') }}">Administration</x-dropdown-link>

            @if (Auth::user()->is_instance_admin)
              <x-dropdown-link href="{{ route('instance.index') }}">Instance administration</x-dropdown-link>
            @endif
    ...
    @else
      @if (config('peopleos.enable_waitlist'))
        <div class="flex items-center gap-x-5">
          <a href="{{ route('waitlist.index') }}" class="rounded-md bg-blue-600 px-3.5 py-1.5 text-sm font-semibold text-white shadow-sm hover:bg-blue-500 focus-visible:outline focus-visible:outline-offset-2 focus-visible:outline-blue-600">Join waitlist</a>
        </div>
      @else
        <div class="flex items-center gap-x-5">
  ...
  <div x-show="mobileMenuOpen" class="lg:hidden" style="display: none">
    ...
    <div class="fixed inset-y-0 right-0 z-50 w-full overflow-y-auto bg-white px-6 py-6 sm:max-w-sm sm:ring-1 sm:ring-gray-900/10">
      ...
      <div class="flex flex-col gap-y-4">
        ...
        <a href="{{ route('marketing.pricing.index') }}" class="flex items-center gap-x-2 py-2 text-base leading-7 font-semibold text-gray-900">Pricing</a>
        <a href="{{ route('marketing.docs.index') }}" class="flex items-center gap-x-2 py-2 text-base leading-7 font-semibold text-gray-900">Docs</a>
        <a href="" class="flex items-center gap-x-2 py-2 text-base leading-7 font-semibold text-gray-900">Community</a>
        <a href="{{ route('marketing.company.index') }}" class="flex items-center gap-x-2 py-2 text-base leading-7 font-semibold text-gray-900">Company</a>
      </div>
    </div>
  </div>
</div>
